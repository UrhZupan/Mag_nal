...
class UNet3(nn.Module):
    def __init__(self):
        ...
        self.down_conv_3 = double_conv(128 + 1, 256) # + 1 --> 2ND INPUT
        ...
        # Last convolution
        self.out_conv = nn.Conv2d(64, 1, kernel_size=1) # Our output has 1 channel
    ...
    def forward(self, rgb_image, depth_image_down):
        ...
        x0 = torch.cat([x4, depth_image_down], dim=1)
        #############################################
        
        x5 = self.down_conv_3(x0) 
...
#model = UNet3().to(device='cuda' if torch.cuda.is_available() else 'cpu')
#output1 = model.forward(rgb_input, depth_input)
#print("Output tensor size: ", output1.size())

#"""output size in pix: 384 x 520"""